...
namespace cuda {
	...
	inline void matrix_sum0(const _Mat& A, _Vec& V) {
		...
		_matrix_sum0<_T1, _T2>(A.data(), V.data(), N, M);
	}
	
	template <class _Mat, class _Vec>
		inline void matrix_sum1(const _Mat& A, _Vec& V) {
		constexpr bool _T1 = A.is_transposed();
		constexpr bool _T2 = V.is_transposed();
		...
		_matrix_sum1<_T1, _T2>(A.data(), V.data(), N, M);
	}
}